...
static ssize_t secfs_me_read(struct file *file, char __user *ubuf, size_t count,
			     loff_t *ppos)
{
	...
	return simple_read_from_buffer(ubuf, count, ppos, buffer,
				       strlen(buffer));
}
			       ...
			       size_t count, loff_t *ppos)
{
	...
	snprintf(buffer, sizeof(buffer), "%lld %d %s %s %s %s",
		 request.request_id, from_kuid(current_user_ns(), request.uid),
...
static int __init trustedcell_secfs_init(void)
{
	...
	securityfs_create_file("status", 0666, directory, NULL,
			       &secfs_status_ops);
	securityfs_create_file("me", 0666, directory, NULL, &secfs_me_ops);
	securityfs_create_file("host", 0600, directory, NULL, &secfs_host_ops);
	return 0;
}
