...
static uint32_t enforce_cache_hashfn(struct trustedcell_enforce_cache_item item)
{
	uint32_t result = 0;
	result ^= hsiphash(&item.uid, sizeof(item.uid),
			   &enforce_cache.hasher_key);
	result ^= hsiphash(item.cell->str, strlen(item.cell->str),
			   &enforce_cache.hasher_key);
	result ^= hsiphash(item.category, strlen(item.category),
			   &enforce_cache.hasher_key);
	if (item.category[0] == '~') {
		result ^= hsiphash(item.owner, strlen(item.owner),
				   &enforce_cache.hasher_key);
	}
	result ^= hsiphash(item.action, strlen(item.action),
			   &enforce_cache.hasher_key);
	return result;
}

static bool
enforce_cache_item_match(struct trustedcell_enforce_cache_item *pattern,
			 struct trustedcell_enforce_cache_item *obj)
{
	bool p = pattern->uid == obj->uid &&
		 ...
		 (pattern->category[0] == '~' ?
			  strcmp(pattern->owner, obj->owner) == 0 :
			  true) &&
...
int trustedcell_enforce_cache_add(struct trustedcell_enforce_cache_item item)
{
	...
	uint32_t slot =
		enforce_cache_hashfn(item) % TRUSTEDCELL_ENFORCE_CACHE_SLOTS;
...
{
	uint32_t slot =
		enforce_cache_hashfn(mat) % TRUSTEDCELL_ENFORCE_CACHE_SLOTS;
	struct enforce_cache_node *i;
	rcu_read_lock();
